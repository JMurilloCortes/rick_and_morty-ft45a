...
describe("Test de RUTAS", () => {
    ...
    describe("GET /rickandmorty/login", () => {
        it("Retorna objeto { access: true } con credenciales correctas", async () => {
            const response = await agent.get(
                "/rickandmorty/login?email=jealmuco@gmail.com&password=Juanes.6"
            );
            expect(response.body).toEqual({ access: true });
        });
        it("Retorna objeto { access: false } con credenciales incorrectas", async () => {
            const response = await agent.get(
                "/rickandmorty/login?email=jealmuco@gmail.com&password=Juanes"
            );
    ...
    describe("DELETE /rickandmorty/fav/:id", () => {
        it("Devuelve array con pesonajes si no elimina ningún personaje", async () => {
            const response = await agent.delete("/rickandmorty/fav/15");
            expect(response.body).toContainEqual(character1);
            expect(response.body).toContainEqual(character2);
        });
        it("Devuelve array sin el pesonajes del id enviado", async () => {
